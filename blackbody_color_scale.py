...
dv = (g[1] - g[0])/2
nv = int(ceil(r[0]/dv))
nr = [max(0,r[0]-dv*i) for i in range(nv)]
ng = [max(0,g[0]-dv*i) for i in range(nv)]
nb = [max(0,b[0]-dv*i) for i in range(nv)]
nr.reverse()
ng.reverse()
nb.reverse()
r = nr + r
    ...
    if crawling_chaos:
        ...
        for i in range(img.size[0]):
            for j in range(img.size[1]):
                k = ((randint(0, sq) * randint(0, sq)) // 100) % n
                pixels[i,j] = (r[k], g[k], b[k])
    filepath = os.path.join(foldername, 'frame_{:04}.gif'.format(p))
    #print(filepath)
...
imageio.mimsave('fire.gif', images, duration = 0.1)
shutil.rmtree(foldername)
